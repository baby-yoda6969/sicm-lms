...
export default function StudentDailyTimetablePage() {
  ...
  const [dailySchedule, setDailySchedule] = useState<DailyTimetablePeriod[]>([]);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-stone-200/80">
          <div>
            ...
            <p className="text-xs text-stone-500 font-medium mt-0.5">
              Official schedule generated by Dean & Administration for <strong>{user?.sectionName || 'BCA 2nd Year'}</strong> on <strong>{selectedDate}</strong> ({generationMeta?.dayOfWeek})
            </p>
          </div>
                ...
                {dailySchedule.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-14 text-center text-stone-400">
                      ...
                      <p className="text-xs text-stone-400 mt-0.5">Admin has not scheduled lectures on this date.</p>
                    </td>
                  </tr>
                ) : (
                  dailySchedule.map((c) => {
